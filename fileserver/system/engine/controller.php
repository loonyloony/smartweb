...
abstract class Controller {
	...
	public function render() {		
		...
		foreach ($this->children as $child) {
			...
			if (file_exists($file)) {
				...
				exit('Error: Could not load controller1 ' . $child . '!');
			}
		}
		...
		if ($this->layout) {
			...
			if (file_exists($file)) {
				...
				exit('Error: Could not load controller2 ' . $this->layout . '!');
			}	
		}
	...
	protected function fetch($filename) {
		if(file_exists(DIR_USERTEMPLATE.$filename))
		{
			$file = DIR_USERTEMPLATE . $filename;	
		}
		else
		{
	...
	public function loadModule($module, $method="index", $args = array())
	{
		...
		if (file_exists($file)) {
			...
			//exit('Error: Could not load controller3 ' . $module . '!');
	...
	{	
		if (file_exists($file)) {
			...
			exit('Error: Could not load controller4 ' . $child . '!');
		}
