...
var countfix = 0;
function chrometimefix(top, opt)
{
...
function pqtuanfix(top, opt)
{
	opt.cd = opt.cd + 1;
	//$("#testslide").html(opt.cd + "/" + opt.timer);
	var slidemask = top.find('.slide_mainmask');
	opt.loaded = 1;
	//throw( new Error("opt.loaded : "+opt.loaded, "") );
	//chrometimefix(top, opt);
	if (opt.loaded == 1) {
	  ...
	  if (!slidemask.hasClass('overme') && !slidemask.hasClass('videoon')) {
		...
		//throw( new Error("AAA : "+opt.cd +"/"+ opt.timer, "") );
		if (opt.cd == opt.timer) {
		  opt.cd = 0;
		  if (newitem.index() < opt.maxitem - 1) {
	...
	/*alert("aaa"+countfix+"/"+opt.cd);
	if(countfix == 2 && opt.cd == 1)
	{
		countfix == 0;
		setTimeout(function () {
			  pqtuanfix(top,opt)
