...
<script>
	...
	function save(){
			showloading();
			var url="?route=module/videoalbum/save";
			var arr_language=<?php echo json_encode($arr_language) ?>;
			for(var x in arr_language){
			...
			$.post(url,$('#frmform').serialize(),function(datas){
					...
					if(arrs[0]=="true")	{
						...
						window.location=HTTP_SERVER+"?route=module/videoalbum/insert&sitemapid="+arrs[2]+"&mediaid="+arrs[1]+"#tabedit";
	...
	function Attachment()
	{
		...
		this.addImageSub=function(iid,name,path,thums){
				...
				var img="<div class='image_item'><a rel='"+path+"'><img class='video-image-contact' src='<?php echo HTTP_SERVER.DIR_IMAGE ?>button/next.png' />"+name+"</a><input type='hidden' name='subfileid[]'  value='"+iid+"' /><input type='hidden' name='subfilepath[]'  value="+path+" /></div>";
