...
<table class="data-table " style="margin-top:10px;" id="download">
    ...
    <tr>
        <td>
            ...
            <a onclick="elifeupload_onefile('filepath<?php echo $index?>','fileurlpreview<?php echo $index?>')" class="button"><?php echo $button_selectimage ?></a>
        </td>
...
function uploadMultiFileDownload(filetype){
	...
	var img="<div class='file_item' style='margin-bottom:10px;'><a class='uploadfilelink'><img class='video-image-contact' src='<?php echo HTTP_SERVER.DIR_IMAGE ?>button/"+imagetype+".png' /><span id=fileurlpreview"+index+"></span><br /><input type='hidden' name='subfileid[]'   />"
	img +="<input id=filepath"+index+"  type='hidden' name='subfilepath[]'  /> <div class='clearer'></div></div> <a onclick=elifeupload_onefile('filepath"+index+"','fileurlpreview"+index+"') class='button'><?php echo $button_selectimage ?></a>";
