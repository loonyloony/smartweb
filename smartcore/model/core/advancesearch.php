...
class ModelCoreAdvanceSearch extends Model {
	...
	public function getListByKeyword($where="", $keyword, $from=0, $to=0)
	{
		$common = Registry::get('common');
		
		$escapeKeyword = $common->utf8_to_ascii($keyword);
		
 		$languageid = $this->config->get('config_languageid');
		
		$sql =  "SELECT title, imagepath, summary, updateddate, seo_url, objtype".
				"	FROM advance_search" .
				"	WHERE languageid = '". $languageid ."' AND (objtype = 'media') AND (seo_url != '') AND (rootcontent LIKE '%$keyword%' OR rootcontent LIKE '%$escapeKeyword%'".
				"		OR escapecontent LIKE '%$escapeKeyword%'".				
				"		OR alt_image LIKE '%$keyword%' OR alt_image LIKE '%$escapeKeyword%'".
				"		OR others LIKE '%$keyword%' OR others LIKE '%$escapeKeyword%') ". $where;
