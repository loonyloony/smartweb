<?php
class ControllerModuleGroupproduct extends Controller
{

    private $sitemapgroup = "";

    public function index()
    {
        $sitemapgroup = $this->smartweb->sitemap['sitemapid'] . ",";
        $sitemapgroup .= $this->setSitemap($this->smartweb->sitemap['sitemapid']);
        if (substr($sitemapgroup, -1) == ",") {
            $sitemapgroup = substr($sitemapgroup, 0, -1);
        }
        $this->sitemapgroup = $sitemapgroup;
    ...
    private function setSitemap($parentId)
    {
        $where = "";
        $listSitemap = $this->model_core_sitemap->getList(SITEID, "AND sitemap.sitemapparent = " . $parentId . "");
        if (count($listSitemap) > 0) {
            foreach ($listSitemap as $item) {
                $where .= $item['sitemapid'] . ",";
                $where .= $this->setSitemap($item['sitemapid']);
            }
        }
        return $where;
    }
    ...
    private function getTags()
    {
        ...
        $where = "AND sitemapid IN (" . $this->sitemapgroup . ") AND language = '" . $this->config->get('config_languageid') . "' AND count > 0";
        $this->data['tags'] = $this->model_core_tag->getList($where);
    }

    private function getListMonth()
    {
        //get month list
        $listmonth = $this->model_core_product->getListMonth(" AND sitemapid IN ( " . $this->sitemapgroup . " )", 0, 5);
    ...
    {
        //get month list
        $listManArea = $this->model_core_product->getListManArea(" AND p.sitemapid IN ( " . $this->sitemapgroup . " )", true);
    ...
    private function getList()
    {
        ...
        $where .= " AND p.sitemapid IN ( " . $this->sitemapgroup . " )";
