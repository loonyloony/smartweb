...
class ControllerModuleVideo extends Controller{
	...
	public function index(){
		...
		$arrayOfFiles = $this->model_core_media->getFileByMediaId($this->data['media']['mediaid'],"file");
		
		if(count($arrayOfFiles)){
			foreach($arrayOfFiles as $item){
				$file = array();
				$file['file'] = FILE_SERVER . "smartweb_" . SKINID . "/file/" . $item['filepath'];
				$file['image'] = DIR_IMAGE . "video_icon.jpg";
				//$file['title'] = $item['title'];
				$file['description'] = $item['title'];
