		...
		function save(){
				...
				$.post(url,$('#frmform').serialize(),function(data){
						...
						if(arr[0]=="true")
						{
							...
							window.location=HTTP_SERVER+"?route=module/<?php echo $module ?>/insert&sitemapid="+arr[2]+"&mediaid="+arr[1]+"#tabedit";
							$.unblockUI();
						}else{
							$.unblockUI();
					...
					switch($('#outputtype').val())
					{
						...
						case 'download':
								...
								function(file) 
								{
									...
									$('#'+handler).append(download.addImageSub(data.file.fileid,data.file.filename,data.file.filepath,file.file.imagepreview));
...
function Download()
	{
		...
		this.addImageSub=function(iid,name,path,thums){
				...
				var img="<div class='image_item'><a rel='"+path+"'><img class='video-image-contact' src='<?php echo HTTP_SERVER.DIR_IMAGE ?>button/next.png' />"+name+"</a><input type='hidden' name='downloadfileid[]'  value='"+iid+"' /><input type='hidden' name='downloadfilepath[]'  value="+path+" /></div>";
