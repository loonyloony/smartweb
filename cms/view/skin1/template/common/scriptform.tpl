...
function save(){
		...
		$.post(url,$('#frmform').serialize(),function(datas){
				...
				if(arrs[0]=="true")	{
					...
					window.location=HTTP_SERVER+"?route=module/<?php echo $module ?>/insert&sitemapid="+arrs[2]+"&mediaid="+arrs[1]+"#tabedit";
				}else{
					$.unblockUI();
	...
	function Attachment()
	{
		...
		this.addImageSub=function(iid,name,path,thums){
				...
				var img="<div class='image_item'><img src='"+thums +"'/><input type='hidden' name='subfileid[]'  value='"+iid+"' /><input type='hidden' name='subfilepath[]'  value="+path+" /></div>";
						...
						strlanguage +="<input type='text' class='text left' size='80' name=sub<?php echo $key ?>[] />   <div class='icon_language left'><img class='icon_language' width='32' height='26' src='<?php echo HTTP_SERVER.DIR_IMAGE ?>language/<?php echo $key ?>.png'  /></div><br>";
						strlanguage +="<?php }?>";
