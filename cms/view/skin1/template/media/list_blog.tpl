            ...
            <tbody>
                ...
                ?>
                    <tr class="media-<?php echo $items[$i]['status']?>" class="<?php echo ($index % 2 == 1 ? 'even' : 'even') ?>">
                        ...
                        <a title="Xem hiển thị" href="<?php echo $items[$i]['linkpreview']?>" target="_blank" class="review button">Xem</a>
                        <?php 
                        if($this->user->getUserTypeId() == "admin") {
                        	if($items[$i]['status'] == "pending") { 
                        ?>
                        <a title="Xuất bản" class="button" onclick="publish('<?php echo $items[$i]['mediaid'] ?>','active')">Xuất bản</a>
                        <?php
                        } else {
                        ?>
                        <a title="Tạm ẩn" class="button" onclick="publish('<?php echo $items[$i]['mediaid'] ?>','pending')">Tạm ẩn</a>
                        <?php
                        }}
                        ?>
                        </td>
