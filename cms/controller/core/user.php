...
class ControllerCoreUser extends Controller
{
	...
	public function active()
	{
		...
		if($data['status'] == "active")
			$this->data['output']= $this->language->get('announ_active');
		if($data['status'] == "lock")
			$this->data['output']= $this->language->get('announ_lock');
		$this->id="content";
	...
	private function getList() 
	{
		...
		//for($i=0; $i <= count($this->data['users'])-1 ; $i++)
		{
			...
			if($this->data['users'][$i]['status']=='lock')
				$this->data['users'][$i]['text_active'] = $this->language->get('button_active');
			else
				$this->data['users'][$i]['text_active'] = $this->language->get('button_lock');
				
			if($this->data['users'][$i]['status']=='active')
				$this->data['users'][$i]['text_st'] = $this->language->get('button_active');
			else
				$this->data['users'][$i]['text_st'] = $this->language->get('button_lock');
		}
