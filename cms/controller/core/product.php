...
class ControllerCoreProduct extends Controller
{
	...
	private function getList() 
	{
		...
		if(trim($para['manufacturer']) && $para['manufacturer'] != "undefined")
		{
	...
	public function getFilterOption() {
		...
		if(isset($filterid)) {
			...
			$arrFilterOption = $this->model_product_filteroption->getOptionValues($filterid, $languageid);
			$this->data['output'] = $this->model_common_control->getDataCombobox($arrFilterOption, 'name', 'filter_value_id');
	...
	private function getForm()
	{
		...
		if ((isset($this->request->get['productid'])) ) 
		{
			...
			$this->data['boxmanu'] = $this->model_common_control->getDataCombobox($arrmanu, 'name', 'manufacturerid', $this->data['product']['manufacturerid']);
			...
			$filterOptionSelected = $this->model_product_filteroption->getItem($this->data['product']['filter']);
			$this->data['boxfilter'] = $this->model_common_control->getDataCombobox($filters, 'name', 'filterid', $filterOptionSelected['filterid']);
			if(count($filters) > 0) {
				$firstFilter = $filterOptionSelected['filterid'];
				$languageid = $this->config->get('config_languageid');
				$arrFilterOption = $this->model_product_filteroption->getOptionValues($firstFilter['filterid'], $languageid);
				$this->data['boxfilteroption'] = $this->model_common_control->getDataCombobox($arrFilterOption, 'name', 'filter_value_id', $this->data['product']['filter']);
			}
	...
	private function validateForm($data)
	{
		...
		if(isset($data['product_description'])){
			foreach($data['product_description'] as $key => $description){
				...
				if(!$this->smartweb->checkSEOURL($description['seo_url'], "product", $data['productid'], $key)) {
