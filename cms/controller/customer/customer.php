...
class ControllerCustomerCustomer extends Controller
{
    ...
    public function getForm($label_heading_title)
    {
        ...
        //gender
        $genders = array("Ms." => "Ms.", "Mr." => "Mr.", "Mrs." => "Mrs", "Dr." => "Dr.");
        $this->data['cboGenders'] = $this->model_common_control->getComboboxData("", $genders, "male");
        ...
        $customertypes = array($text_tenant => $text_tenant, $text_landlord => $text_landlord, $text_agent => $text_agent);
        $this->data["cboCustomerType"] = $this->common->combobox($customertypes, "", "", $this->data['customer']['customertype']);
    ...
    public function getList()
    {
        ...
        $this->data['bonusGroupList'] = $this->model_core_bonusgroup->getList();

        $this->data['customers'] = array();

        $filter_customerName = trim(urldecode($_GET['filter_customerName']));
        $filter_bonusGroup = trim($_GET['filter_bonusGroup']);
        $filter_customerid = trim($_GET['filter_customerid']);

        $where = "";
        $customerid = 0;
        if ($filter_customerid != '') {
            if (SKINID == 'fanhanghieu') {
                $prefixCustomerID = "FHH";
                $customerid = (int)str_replace($prefixCustomerID, "", $filter_customerid);
                $where .= " AND (customerid = " . $customerid . ")";
            }
        }

        if ($filter_customerName != '') {
            $where .= " AND (firstname like '%" . $filter_customerName . "%' OR lastname like '%" . $filter_customerName . "%') ";
        }

        if ($filter_bonusGroup != '') {

            $pointMin = 0;
            $pointMax = 999999999;

            $bonusGroup = $this->model_core_bonusgroup->getItem(" AND bonus_group_id = " . $filter_bonusGroup);
            if ($bonusGroup != null) {
                $pointMin = $bonusGroup['point'];
            }

            $bonusGroupItemUpper = $this->model_core_bonusgroup->getItem(" AND point > " . $pointMin);
            if ($bonusGroupItemUpper != null) {
                $pointMax = $bonusGroupItemUpper['point'];
            }

            $where .= " AND (point >= " . $pointMin . " AND point < " . $pointMax . ")";
        }
        ...
        for ($i = $offset; $i < $offset + $limit && count($rows[$i]) > 0; $i++) //for($i=0; $i <= count($this->data['customer_groups'])-1 ; $i++)
        {
    ...
    public function changePassword()
    {
        ...
        $this->id = 'content';
        $this->layout = 'layout/center';
        $this->template = "customer/changepassword.tpl";
    ...
    public function validatePassword($data)
    {
        ...
        //rule Singapore
        $pattern = "/([a-zA-Z0-9]){6,}/";
        if(SKINID == 'singapore' && !preg_match($pattern, $data['password'])) {
            $this->errpassword['password'] = $this->language->get('war_password_rule_6_characters_numbers');
        }
